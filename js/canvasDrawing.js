...
const canvasOriginalCreatorUsername = document.querySelector(
    "#canvas_originalCreatorUsername"
);
const canvasCurrentOwner = document.querySelector("#canvas_currentOwner");
const canvasCurrentOwnerUsername = document.querySelector(
    "#canvas_currentOwnerUsername"
);
...
function RefreshInfo() {
    ...
    xmlHttp.open(
        "POST",
        "../api/refreshCanvasInfo.php?canvasCode=" + canvasCode,
        true
    );
    xmlHttp.send();
}
...
function GenerateCanvasCode() {
    ...
    canvasPixels.forEach((pixel) => {
        ...
        let colorCode =
            ...
            colorClass.substring(colorClass.length - 3, colorClass.length - 2);
...
function CheckIfUserCanAffordCanvas() {
    if (userOwnsCanvas) {
        canvasPurchaseButton.classList.add(
            "bg-neutral-300",
            "cursor-not-allowed"
        );
    ...
    if (currentUserWallet < currentCanvasPrice) {
        ...
        canvasPurchaseButton.classList.remove(
            "bg-neutral-300",
            "cursor-not-allowed"
        );
        currentPurchaseButtonState = "enabled";
...
TryToPurchaseCanvas = () => {
    ...
    xmlHttp.open(
        "POST",
        "../api/purchaseCanvas.php?canvasCode=" +
            GenerateCanvasCode() +
            "&canvasName=" +
            canvasNameField.value,
        true
    );
    xmlHttp.send();
};
