...
function GetTop10Users() {
    ...
    xmlhttp.onload = function() {
        ...
        users.forEach((user, index) => {
            ...
            rankingsTable.innerHTML += 
            `<div id="td_${user.username}" class="group w-full h-10 flex justify-between items-center py-6 px-2 select-none cursor-pointer">
                <p class="w-14 flex items-center justify-center ${color} px-9 p-1 rounded-xl ">#${index + 1}</p>
                <p class="group-hover:scale-105 transition-all">${user.username}</p>
                <p class="w-14 flex items-center justify-center text-white bg-neutral-900/75 px-9 p-1 rounded-xl">${parseInt(user.value).toLocaleString()}</p>
            </div>`;
        });
        users.forEach(user => {
