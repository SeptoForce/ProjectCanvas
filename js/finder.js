    ...
    xmlhttp.onload = function() {
        ...
        if (response.success === "true") {
            ...
            canvases.forEach(canvas => {
                let id = canvas["id"];
                canvasShowcase.innerHTML += 
                `<div id="${"canvas_"+id}" class="hover:scale-105 cursor-pointer select-none transition-all w-56 h-min bg-neutral-100 rounded-xl drop-shadow-[0px_2px_10px_rgb(0,0,0,0.1)] flex flex-col justify-between items-center text-center overflow-hidden">
            ...
            canvasShowcase.innerHTML = "<div><p class='text-neutral-800 text-3xl mt-8'>No canvases found.</p></div";
        }
    }
    xmlhttp.open("POST", "../api/finder.php?sort="+sortType+"&filter="+filterType+minValueURL+maxValueURL, true);
    xmlhttp.send();
}
